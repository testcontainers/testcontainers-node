...
describe("GenericContainer reuse", { timeout: 180_000 }, () => {
  it.concurrent("should not reuse the container by default", async () => {
  ...
  it.concurrent("should not reuse the container even when there is a candidate 1", async () => {
    const name = `there_can_only_be_one_${randomUuid()}`;
    const container = await new GenericContainer("cristianrgreco/testcontainer:1.1.14")
      .withName(name)
      .withExposedPorts(8080)
  ...
  it.concurrent("should not reuse the container even when there is a candidate 2", async () => {
    const name = `there_can_only_be_one_${randomUuid()}`;
    const container = await new GenericContainer("cristianrgreco/testcontainer:1.1.14")
      .withName(name)
      .withExposedPorts(8080)
  ...
  it.concurrent("should not reuse the container when TESTCONTAINERS_REUSE_ENABLE is set to false", async () => {
  ...
  it.concurrent(
    "should create a new container when an existing reusable container has stopped and is removed",
    async () => {
      const name = `there_can_only_be_one_${randomUuid()}`;
      const container1 = await new GenericContainer("cristianrgreco/testcontainer:1.1.14")
        .withName(name)
  ...
  it.concurrent("should reuse stopped container, if configured withAutoRemove(false)", async () => {
  ...
  it.concurrent("should reuse container when an existing reusable container has stopped but not removed", async () => {
    const name = `there_can_only_be_one_${randomUuid()}`;
    const container1 = await new GenericContainer("cristianrgreco/testcontainer:1.1.14")
      .withName(name)
      .withExposedPorts(8080)
  ...
  it.concurrent("should keep the labels passed in when a new reusable container is created", async () => {
    const container = await new GenericContainer("cristianrgreco/testcontainer:1.1.14")
  ...
  it.concurrent("should not create multiple reusable containers if called in parallel", async () => {
