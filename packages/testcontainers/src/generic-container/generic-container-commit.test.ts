...
describe("GenericContainer commit", { timeout: 180_000 }, () => {
  ...
  it("should commit container changes to a new image", async () => {
    ...
    await using container = await new GenericContainer(`${imageName}:${imageVersion}`).withExposedPorts(8080).start();
    ...
    await using newContainer = await new GenericContainer(imageId).withExposedPorts(8080).start();
