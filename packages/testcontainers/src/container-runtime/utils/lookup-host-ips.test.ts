...
describe("lookupHostIps", () => {
  it.concurrent("should return a list of resolved host IPs when host is not an IP", async () => {
  ...
  it.concurrent("should return host IP and v4 family when host is an IPv4 IP", async () => {
    expect(await lookupHostIps("127.0.0.1")).toEqual([{ address: "127.0.0.1", family: 4 }]);
    expect(mockDnsLookup).not.toHaveBeenCalled();
  });

  it.concurrent("should return host IP and v6 family when host is an IPv6 IP", async () => {
