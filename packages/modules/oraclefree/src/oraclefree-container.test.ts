...
describe.sequential("OracleFreeContainer", { timeout: 240_000 }, () => {
    ...
    it("should throw error when using default database name", () => {
        ...
        expect(() => container.withDatabase("FREEPDB1")).toThrow(
            'The default database "FREEPDB1" cannot be used'
        );
    });
    ...
    it('should set the custom database and user', async () => {
        ...
        await using container = await new OracleDbContainer(IMAGE).withDatabase(customDatabase).withUsername(customUsername).withPassword(customPassword).start();
