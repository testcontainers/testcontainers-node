...
import { checkImageExists, composeContainerName, getContainerIds, getRunningNetworkIds } from "./test-helper";
...
describe.skip("Reaper", () => {
  ...
  it("should not remove reusable containers", async () => {
    ...
    try {
      const runningContainerIds = await getContainerIds();
      expect(runningContainerIds).not.toContain(container.getId());
    } finally {
